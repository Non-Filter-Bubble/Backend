    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        ...
        http
                .authorizeHttpRequests((auth) -> auth
                        .requestMatchers("/searchLibraryByBook","/login","/","/search-books","/search-NaverBooks", "/join","/recommend","/user/check-nickname","/user/check-username","/load-books","/bestseller/**", "/swagger-ui/**", "/v3/api-docs/**", "/test/**", "/test" ).permitAll() // 로그인 페이지와 일부 페이지는 모든 사용자에게 허용
